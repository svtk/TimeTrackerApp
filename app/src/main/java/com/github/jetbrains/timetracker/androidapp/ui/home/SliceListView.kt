...
import com.github.jetbrains.timetracker.androidapp.model.toWorkSlicesByDays
...
@Composable
fun SliceListView(
    slicesByDays: WorkSlicesByDays,
    state: LazyListState = rememberLazyListState(),
    onCardClicked: (id: UUID) -> Unit,
) {
    LazyColumn(state = state) {
        slicesByDays.entries.forEach { (timeRange, slices) ->
            item(key = timeRange.title) {
                Subtitle(
                    modifier = Modifier.padding(top = 20.dp),
                    text = timeRange.title
                )
            }
            items(slices, key = WorkSlice::id) { slice ->
                SliceCard(
                    slice = slice,
...
fun SliceListPreview() {
    TimeTrackerAppTheme {
        SliceListView(
            slicesByDays = createTestSlices().toWorkSlicesByDays(),
            onCardClicked = {},
        )
    }
}
